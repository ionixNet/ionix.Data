...
namespace ionixTests.SqlServer
{
    public static class DataFactory
    {
                    ...
                    lock (syncRoot)
                    {
                        if (null == _OnExecuteSqlComplete)
                        {
                            ...
                            _OnExecuteSqlComplete = (e) =>
                            {
                                try
                                {
                                    using (Stream fs = new FileStream("x:\\sql.txt", FileMode.Append, FileAccess.Write))
                                    {
        ...
        public static DbConnection CreateDbConnection()
        {
            ...
            conn.ConnectionString = ConfigurationManager.ConnectionStrings["SqlConn"].ConnectionString;// Data Source=.;Initial Catalog=Syncaride;User Id=admin;Password=1;";
            conn.Open();
        ...
        public static DbClient CretDbClient()
        {
            return new DbClient();
        }
