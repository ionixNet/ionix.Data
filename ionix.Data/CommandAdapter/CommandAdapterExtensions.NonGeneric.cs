namespace ionix.Data
{
    ...
    public static partial class CommandAdapterExtensions
    {
        ...
        public static object SelectById(this ICommandAdapter adapter, Type entityType, params object[] idValues)
        {
        ...
        private static MethodInfo BatchUpdateMethod(Type genericType)
        {
            if (null == _batchUpdateMethod)
            {
                ...
                {
                    if (null == _batchUpdateMethod)
                        _batchUpdateMethod = ReflectionExtensions.GetMethod(ICommandAdapterType, "BatchUpdate", 3);
                }

            }
            return _batchUpdateMethod.MakeGenericMethod(genericType);
        }
        public static int BatchUpdateNonGeneric(this ICommandAdapter adapter, IEnumerable entityList, BatchCommandMode mode, params string[] updatedFields)
        {
            object first = IsBatchValid(adapter, entityList);
            if (null != first)
            {
                Type entityType = first.GetType();
                return (Int32)BatchUpdateMethod(entityType).Invoke(adapter, new object[] { entityList, mode, updatedFields });
            }
            return 0;
        }
        ...
        private static MethodInfo BatchInsertMethod(Type genericType)
        {
            if (null == _batchInsertMethod)
            {
                ...
                {
                    if (null == _batchInsertMethod)
                        _batchInsertMethod = ReflectionExtensions.GetMethod(ICommandAdapterType, "BatchInsert", 3);
                }

            }
            return _batchInsertMethod.MakeGenericMethod(genericType);
        }
        public static int BatchInsertNonGeneric(this ICommandAdapter adapter, IEnumerable entityList, BatchCommandMode mode, params string[] insertFields)
        {
            object first = IsBatchValid(adapter, entityList);
            if (null != first)
            {
                Type entityType = first.GetType();
                return (Int32)BatchInsertMethod(entityType).Invoke(adapter, new object[] { entityList, mode, insertFields });
            }
            return 0;
        }
        ...
        private static MethodInfo BatchUpsertMethod(Type genericType)
        {
            if (null == _batchUpsertMethod)
            {
                ...
                {
                    if (null == _batchUpsertMethod)
                        _batchUpsertMethod = ReflectionExtensions.GetMethod(ICommandAdapterType, "BatchUpsert", 4);
                }

            }
            return _batchUpsertMethod.MakeGenericMethod(genericType);
        }
        public static int BatchUpsertNonGeneric(this ICommandAdapter adapter, IEnumerable entityList, BatchCommandMode mode, string[] updatedFields, string[] insertFields)
        {
            object first = IsBatchValid(adapter, entityList);
            if (null != first)
            {
                Type entityType = first.GetType();
                return (Int32)BatchUpsertMethod(entityType).Invoke(adapter, new object[] { entityList, mode, updatedFields, insertFields });
            }
            return 0;
        }
    }
